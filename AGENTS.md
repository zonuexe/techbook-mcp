...
- **旧刊カバレッジの限界**: アダプタは現行ストアの生カタログをスクレイプするため、販売終了・取り下げ済みの旧刊（2008-13頃の短編・電子書籍専売）は構造的に `search_books` でヒットしない。O'Reilly 旧刊（例 `CSS3の値、単位、色` 9784873116266）は `/ebook/` 一覧・`/catalog/`・sitemap いずれにも無く openBD も未登録。詳細は `docs/design-doc.md`「カバレッジの制約」。**部分救済**: 詳細ページが ISBN ベースの安定URL（O'Reilly `/books/{isbn}/`）を持つ出版社は `PublisherAdapter.detailUrlForIsbn?(isbn)` を実装でき、`get_book_by_isbn` が openBD ミス時にカーリルより先に詳細ページを直引きする（`fetchDetailByIsbnCode`、O'Reilly のみ実装済。価格は取れないが書誌は回収）
